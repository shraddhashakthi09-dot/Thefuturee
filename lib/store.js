...
//   - a webhook that the gateway delivers more than once being acted on twice
...
// Webhook event de-duplication only needs to survive the gateway's own
// retry window, so 7 days is comfortably generous.
...
// ── Razorpay order id index ──
// Razorpay's webhook only reports RAZORPAY's own order id (e.g. "order_
// ABC123") — not the orderId this backend generates and uses everywhere
// else (create-order, verify-payment, order-status, the frontend). This
// tiny index lets the webhook look up "which of our orders is this?"
// Written once at create-order time, read once when a webhook arrives.
const RZP_ORDER_INDEX_PREFIX = 'tf_rzp_order_index:';

export async function mapRazorpayOrderId(razorpayOrderId, orderId) {
  await getRedis().set(RZP_ORDER_INDEX_PREFIX + razorpayOrderId, orderId, {
    ex: ORDER_TTL_SECONDS,
  });
}

export async function getOrderIdByRazorpayOrderId(razorpayOrderId) {
  const raw = await getRedis().get(RZP_ORDER_INDEX_PREFIX + razorpayOrderId);
  if (!raw) return null;
